...
#starts new instance of BAC0, attaches to an existing network interface
#python rooms_simulator.py --address 192.168.1.30/24 -p 47808 -di 660011
...
parser.add_argument("-a", "--address", default=None, help="IPV4 address of device interface with network mask, "
                                                          "e.g. 10.0.0.34/24")
parser.add_argument("-p", "--port", default=47808, type=int, help="Port of BACnet device that will be created")
...
#device = connect(ip=args["address"], port=args["port"], deviceId=args["deviceId"]) #use full version with web
